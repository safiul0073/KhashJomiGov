...
class UnoController extends Controller
{
    public function index (Request $request,QueryService $service) {
        ...
        $grohonData = $service->queryCountUpazila($user, $user->role_id, 1);
        $preronData = $service->queryCountUpazila($user, 4,$user->role_id);
        $nothiCount = $user->upazila->bondobosto_apps()->where('status', 1)->count();
        if($tab == 'get1') {
            $applications = $service->queryDataUpazila($user, $user->role_id, 1);
        }else if($tab == 'put1') {
            $applications = $service->queryDataUpazila($user, 4,$user->role_id);
        }else if($tab == 'nothi') {
