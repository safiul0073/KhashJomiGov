...
class TowshilderController extends Controller
{
    public function index (Request $request,QueryService $service ) {
        ...
        $grohonApps = $service->queryCountUnion($user, $user->role_id, 1);
        $preronApp =$service->queryCountUnion($user, 1, $user->role_id);
        $nothiCount = $user->union->bondobosto_apps()->where('status', 1)->count();
        if($tab == 'get1') {
            $applications = $service->queryDataUnion($user, $user->role_id,1);
        }else if($tab == 'put1') {
            $applications = $service->queryDataUnion($user, 1, $user->role_id);
        }else if($tab == 'nothi') {
            $applications = $user->union->bondobosto_apps()->with(['union','upa_zila'])->where('status', 1)->get();
